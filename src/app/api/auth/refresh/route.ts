...
export async function POST() {
  ...
  if (!refreshToken) {
    return NextResponse.json({ error: '인증 토큰이 없습니다, 재로그인 해주세요.' }, { status: 401 })
  }
  ...
  if (!tokenRecord) {
    return NextResponse.json({ error: '유효하지 않은 토큰입니다, 재로그인 해주세요.' }, { status: 401 })
  }
  ...
  try {
    ...
    return NextResponse.json({ error: '토큰 검증에 실패했습니다, 재로그인 해주세요.' }, { status: 401 })
  }
}
